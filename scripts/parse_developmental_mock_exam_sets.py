...
TEXT_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\developmental-psychology-mock-exam-from-pdf.txt")
CSV_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\developmental-psychology-mock-exam-import-exam-sets.csv")
REPORT_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\developmental-psychology-mock-exam-import-exam-sets-report.txt")

SUBJECT = "Developmental Psychology"
EXPECTED_EXAMS = list(range(1, 12))
...
def exam_anchors(text: str) -> list[tuple[int, int]]:
    return [(int(match.group(1)), match.start()) for match in re.finditer(r"(?mi)^EXAM\s+(\d+)\s*$", text)]


def exam_blocks(text: str) -> dict[int, str]:
    anchors = exam_anchors(text)
    blocks: dict[int, str] = {}
    for index, (exam_number, start) in enumerate(anchors):
        end = anchors[index + 1][1] if index + 1 < len(anchors) else len(text)
        blocks[exam_number] = text[start:end]
    return blocks
...
def parse_question(question_number: int, block: str) -> dict[str, str | int]:
    ...
    cleaned = re.sub(r"\nDEVELOPMENTAL PSYCHOLOGY EXAM \d+\n", "\n", cleaned, flags=re.IGNORECASE)
...
def write_report(lines: list[str], total_rows: int) -> None:
    content = ["Developmental Psychology mock exam PDF import report", f"Total exported rows: {total_rows}", ""]
    content.extend(lines)
    content.append("Chapter and Topic were both set to `Exam N` because you requested exam-set naming with no subtopic.")
