...
def exam_anchors(text: str) -> list[tuple[int, int, int]]:
    return [
        ...
        for match in re.finditer(r"(?mi)^ASSESSMENT EXAM\s+(\d+)\s*$", text)
    ]
...
def split_question_blocks(block: str) -> list[tuple[int, str]]:
    matches = list(re.finditer(r"(?m)^\s*((?:[1-9]|[1-9]\d|100))\.\s+[A-Z]", block))
...
def build_rows(text: str) -> tuple[list[list[str | int]], list[str]]:
    ...
    for exam_number in EXPECTED_EXAMS:
        parsed_rows, failed_numbers = parse_exam(exam_number, blocks[exam_number])
        ...
        report.append(f"{exam_label}: {len(parsed_rows)} recovered questions")
        if failed_numbers:
            failed_joined = ", ".join(str(number) for number in failed_numbers)
            report.append(f"{exam_label} missing question stems in source PDF: {failed_joined}")
...
def write_report(lines: list[str], total_rows: int) -> None:
    ...
    content.append("Rows were exported only when the full question stem and all choices were recoverable from the source PDF or OCR.")
