...
TEXT_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\abnormal-psychology-mock-exam-from-pdf.txt")
CSV_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\abnormal-psychology-mock-exam-import-exam-sets.csv")
REPORT_PATH = Path(r"C:\Users\Louie\Desktop\pyschprep\abnormal-psychology-mock-exam-import-exam-sets-report.txt")
...
def exam_anchors(text: str) -> list[tuple[int, int]]:
    return [(int(match.group(1)), match.start()) for match in re.finditer(r"(?mi)^PSYCHOLOGY EXAM\s+(\d+)\s*$", text)]
...
def parse_question(question_number: int, block: str) -> dict[str, str | int]:
    ...
    cleaned = re.sub(r"\nMOCK EXAM- ABNORMAL PSYCHOLOGY EXAM \d+\n", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\nMOCK EXAM- ABNORMAL\n", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\nPSYCHOLOGY EXAM \d+\n", "\n", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\nEXAM \d+\n", "\n", cleaned, flags=re.IGNORECASE)
...
def write_report(lines: list[str], total_rows: int) -> None:
    content = ["Abnormal Psychology mock exam PDF import report", f"Total exported rows: {total_rows}", ""]
